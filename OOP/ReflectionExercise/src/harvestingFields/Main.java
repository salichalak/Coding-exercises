...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        for(Field field : clazz.getDeclaredFields()) {
            fieldsByModifiers.get(Modifier.toString(field.getModifiers())).add(field);
    ...
    public static String formatFieldOutput(Collection<Field> fields) {
        return fields.stream().map(f -> String.format("%s %s %s",
                Modifier.toString(f.getModifiers()),
                f.getType().getSimpleName(),
                f.getName())).collect(Collectors.joining(System.lineSeparator())).trim();
    }
}