...
public class Pirates {
    public static void main(String[] args) {
        ...
        while (!input.equals("Sail")) {
            String[] cityDataParts = input.split("\\|\\|");
            String city = cityDataParts[0];
            int population = Integer.parseInt(cityDataParts[1]);
            int gold = Integer.parseInt(cityDataParts[2]);

            if (cities.containsKey(city)) {
                List<Integer> currentCityData = cities.get(city);
                ...
                cities.put(city, currentCityData);
            } else {
        ...
        while (!input.equals("End")) {
            ...
            switch (command) {
                case "Plunder":
                    ...
                    int peopleBeforePlunder = cities.get(city).get(0);
                    int goldBeforePlunder = cities.get(city).get(1);
                    ...
                    System.out.printf("%s plundered! %d gold stolen, %d citizens killed.%n", city, goldStolen, peopleKilled);
                    cities.put(city, Arrays.asList(remainingPeople, remainingGold));
                    break;
                ...
                case "Prosper":
                    ...
                    int goldBeforeProsper = cities.get(city).get(1);
                    int goldAfterProsper = goldBeforeProsper + goldProsper;
                    cities.get(city).set(1, goldAfterProsper);
                    System.out.printf("%d gold added to the city treasury. %s now has %d gold.%n", goldProsper, city, goldAfterProsper);
                    break;
            }
